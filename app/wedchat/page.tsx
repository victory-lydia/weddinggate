...
const MessageBubble = ({ message }: { message: Message }) => (
  <div className={`flex ${message.type === "user" ? "justify-end" : "justify-start"}`}>
    <div className={`flex items-start space-x-2 max-w-xs lg:max-w-md ${message.type === "user" ? "flex-row-reverse space-x-reverse" : ""}`}>
      <div className={`w-8 h-8 rounded-full flex items-center justify-center ${message.type === "user" ? "bg-blue-500" : "bg-indigo-500"}`}>
      ...
      <div className={`rounded-lg p-3 ${message.type === "user" ? "bg-blue-500 text-white" : "bg-gray-100 text-gray-900"}`}>
        ...
        {message.links?.map((link, index) => (
          <Link key={index} href={link.url}>
            <Button variant="outline" size="sm" className="w-full justify-between text-xs bg-white hover:bg-gray-50 mt-2">
              {link.text}
              <ExternalLink className="h-3 w-3" />
            </Button>
          </Link>
...
export default function WedChatPage() {
  const [messages, setMessages] = useState<Message[]>([
    {
      id: "1",
      type: "assistant",
      content: "Hello! I'm WedChat, your AI assistant for WeddingGate...",
      timestamp: new Date(),
    },
  ]);
  const [inputMessage, setInputMessage] = useState("");
  ...
  // ... (keep your existing findRelevantPages and generateResponse functions)

function generateResponse(input: string): { content: string; links?: { text: string; url: string }[] } {
  const lowerInput = input.toLowerCase()

  for (const key in weddingGateKnowledge) {
    const { description, url, keywords } = weddingGateKnowledge[key]
    if (keywords.some((keyword) => lowerInput.includes(keyword))) {
      return {
        content: description,
        links: [{ text: "Go to page", url }],
      }
    }
  }
